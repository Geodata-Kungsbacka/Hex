CREATE OR REPLACE FUNCTION public.hex_sakerstall_gid_primarnyckel(
    p_schema_namn text,
    p_tabell_namn text
)
    RETURNS text
 ...
 * IDENTITY" utan PRIMARY KEY – hex_aterskapa_tabellregler() tar medvetet bort
 ...
 *
 *   2. Dubbletter kan uppstå tyst. hex_tvinga_gid_fran_sekvens() jämför klientens
 ...
 *     tabellen lämnas orörd – kör public.hex_reparera_gid_dubbletter() manuellt.
 *
 ...
 * att röra tabellen. Planera därför den första körningen av hex_underhall()
...
BEGIN
    ...
    IF seq_namn IS NOT NULL THEN
        ...
        IF max_gid IS NOT NULL THEN
            ...
            IF max_gid > seq_last OR (max_gid = seq_last AND NOT seq_called) THEN
                ...
                RAISE NOTICE '[hex_sakerstall_gid_primarnyckel] Sekvens % framflyttad till %',
                    seq_namn, max_gid;
            END IF;
        END IF;
    END IF;
    ...
    IF antal_dubbl > 0 THEN
        ...
        RAISE WARNING '[hex_sakerstall_gid_primarnyckel] %.% har % gid-värden med dubbletter. '
            'Ingen nyckel lagd. Kör: SELECT * FROM public.hex_reparera_gid_dubbletter(%, %, true);',
    ...
    BEGIN
        IF har_pk THEN
            ...
            -- UNIQUE/PK som återskapats av hex_aterskapa_tabellregler). Ett unikt
            -- index räcker för både QGIS och dubblettskyddet.
            EXECUTE format('ALTER TABLE %I.%I ADD UNIQUE (gid)',
                p_schema_namn, p_tabell_namn);
            RAISE NOTICE '[hex_sakerstall_gid_primarnyckel]   ✓ UNIQUE (gid) skapad på %.%',
                p_schema_namn, p_tabell_namn;
            RETURN 'unik skapad';
        ELSE
            EXECUTE format('ALTER TABLE %I.%I ADD PRIMARY KEY (gid)',
                p_schema_namn, p_tabell_namn);
            RAISE NOTICE '[hex_sakerstall_gid_primarnyckel]   ✓ PRIMARY KEY (gid) skapad på %.%',
                p_schema_namn, p_tabell_namn;
            RETURN 'skapad';
        END IF;
    EXCEPTION WHEN OTHERS THEN
        ...
        RAISE WARNING '[hex_sakerstall_gid_primarnyckel] Kunde inte skapa nyckel på %.%: %',
...
BEGIN
    EXECUTE format(
        'ALTER FUNCTION public.hex_sakerstall_gid_primarnyckel(text, text) OWNER TO %I',
        public.hex_systemagare()
    );
END;
$$;

COMMENT ON FUNCTION public.hex_sakerstall_gid_primarnyckel(text, text)
...
hex_reparera_gid_dubbletter() först.';
