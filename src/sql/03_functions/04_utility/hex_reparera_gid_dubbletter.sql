-- HEX-MIGRERING 2026-08: dubbletter i gid kan bara ha uppstått i tabeller som
-- skapades innan hex_sakerstall_gid_primarnyckel() lade på det unika indexet.
-- Med nyckeln på plats avvisar databasen dem. Tas bort när samtliga fyra
-- produktionsdatabaser kört --upgrade med den här versionen och
-- hex_underhall() rapporterar noll 'dubbletter: N'.
CREATE OR REPLACE FUNCTION public.hex_reparera_gid_dubbletter(
    p_schema_namn text,
    p_tabell_namn text,
 ...
 * Där kan hex_tvinga_gid_fran_sekvens() ha släppt igenom ett klientvalt gid som
 ...
 *
 * ANVÄNDNING
 *   SELECT * FROM public.hex_reparera_gid_dubbletter('sk1_kba_geo', 'vagar');
 *   SELECT * FROM public.hex_reparera_gid_dubbletter('sk1_kba_geo', 'vagar', true);
 *   SELECT public.hex_sakerstall_gid_primarnyckel('sk1_kba_geo', 'vagar');
 ******************************************************************************/
DECLARE
    tabell_oid  oid;
    r           record;
...
BEGIN
    ...
    IF tabell_oid IS NULL THEN
        RAISE EXCEPTION '[hex_reparera_gid_dubbletter] Tabellen %.% finns inte',
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '[hex_reparera_gid_dubbletter] Tabellen %.% saknar gid-kolumn',
            p_schema_namn, p_tabell_namn;
    END IF;
    ...
    IF NOT p_utfor THEN
        RAISE NOTICE '[hex_reparera_gid_dubbletter] Torrkörning. Kör om med '
    ...
    IF antal_kvar > 0 THEN
        RAISE EXCEPTION '[hex_reparera_gid_dubbletter] % dubblettgrupper kvarstår i %.% '
    ...
    RAISE NOTICE '[hex_reparera_gid_dubbletter] %.% är dubblettfri. Kör nu: '
...
BEGIN
    EXECUTE format(
        'ALTER FUNCTION public.hex_reparera_gid_dubbletter(text, text, boolean) OWNER TO %I',
        public.hex_systemagare()
    );
END;
$$;

COMMENT ON FUNCTION public.hex_reparera_gid_dubbletter(text, text, boolean)
...
sekvensvärde. Körs före hex_sakerstall_gid_primarnyckel() på tabeller där denna
