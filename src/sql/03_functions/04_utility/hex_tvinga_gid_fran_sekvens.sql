 ...
 * återuppbyggs automatiskt om sekvensen bytt namn (hex_uppdatera_sekvensnamn).
 ******************************************************************************/
DECLARE
    guc_namn  text;
    cache     text;
...
BEGIN
    ...
    EXCEPTION
        ...
        WHEN undefined_table THEN
            ...
            -- av hex_uppdatera_sekvensnamn under sessionens gång). Slå upp på nytt.
            seq_namn := pg_get_serial_sequence(
                quote_ident(TG_TABLE_SCHEMA) || '.' || quote_ident(TG_TABLE_NAME),
                'gid'
            );
...
BEGIN
    EXECUTE format(
        'ALTER FUNCTION public.hex_tvinga_gid_fran_sekvens() OWNER TO %I',
...
hex_hantera_ny_tabell() på alla Hex-hanterade tabeller.';
