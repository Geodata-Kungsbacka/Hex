CREATE OR REPLACE FUNCTION public.hex_notifiera_gs_borttagning()
 ...
 *
 * Speglar hex_notifiera_gs() men för DROP SCHEMA. Python-lyssnaren tar
...
BEGIN
    RAISE NOTICE E'[hex_notifiera_gs_borttagning] === START ===';
    RAISE NOTICE '[hex_notifiera_gs_borttagning] Kontrollerar om borttaget schema ska avpubliceras från GeoServer';
    ...
    LOOP
        ...
        RAISE NOTICE '[hex_notifiera_gs_borttagning] Bearbetar borttaget schema: %', schema_namn;

        -- Hoppa över systemscheman
        IF schema_namn IN ('public', 'information_schema') OR schema_namn ~ '^pg_' THEN
            RAISE NOTICE '[hex_notifiera_gs_borttagning]   Hoppar över systemschema: %', schema_namn;
            CONTINUE;
        END IF;
        ...
        IF schema_prefix IS NULL THEN
            RAISE NOTICE '[hex_notifiera_gs_borttagning]   Schema "%" har ingen GeoServer-publicerad skyddsnivå - hoppar över', schema_namn;
            CONTINUE;
        END IF;

        -- Skicka borttagningsnotifiering till Python-lyssnaren
        RAISE NOTICE '[hex_notifiera_gs_borttagning]   Skickar borttagningsnotifiering för schema: % (prefix: %)',
            schema_namn, schema_prefix;
        PERFORM pg_notify('geoserver_schema_drop', schema_namn);
        antal_notifieringar := antal_notifieringar + 1;

        RAISE NOTICE '[hex_notifiera_gs_borttagning]   Notifiering skickad till kanal "geoserver_schema_drop"';
    END LOOP;

    RAISE NOTICE '[hex_notifiera_gs_borttagning] Sammanfattning:';
    RAISE NOTICE '[hex_notifiera_gs_borttagning]   Notifieringar skickade: %', antal_notifieringar;
    RAISE NOTICE '[hex_notifiera_gs_borttagning] === SLUT ===';

EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '[hex_notifiera_gs_borttagning] !!! FEL UPPSTOD !!!';
        RAISE NOTICE '[hex_notifiera_gs_borttagning]   Schema: %', COALESCE(schema_namn, 'okänt');
        RAISE NOTICE '[hex_notifiera_gs_borttagning]   Felkod: %', SQLSTATE;
        RAISE NOTICE '[hex_notifiera_gs_borttagning]   Felmeddelande: %', SQLERRM;
        -- Notifieringen är inte kritisk – låt inte felet påverka borttagningen av schemat
        RAISE WARNING '[hex_notifiera_gs_borttagning] GeoServer-borttagningsnotifiering misslyckades, men schemat togs bort korrekt: %', SQLERRM;
END;
$BODY$;
...
BEGIN
    EXECUTE format(
        'ALTER FUNCTION public.hex_notifiera_gs_borttagning() OWNER TO %I',
        public.hex_systemagare()
    );
END;
$$;

COMMENT ON FUNCTION public.hex_notifiera_gs_borttagning()
